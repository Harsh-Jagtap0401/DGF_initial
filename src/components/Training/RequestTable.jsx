    ...
    <TableContainer component={Paper} className="table-container">
      <div className="filters">
        <Tabs value={statuses.indexOf(selectedStatus)} onChange={handleStatusChange} variant="scrollable" scrollButtons="auto">
          {statuses.map(status => (
            <Tab key={status} label={`${status} (${data.filter(row => {
            ...
            <TableRow key={index}>
              ...
              <TableCell>
                <Box display="flex" alignItems="center">
                  {Array.from({ length: Math.min(row.learners, 2) }).map((_, i) => (
                    <Avatar key={i} style={{ marginLeft: i === 0 ? 0 : -12 }}>{row.learners}</Avatar>
                  ))}
                  {row.learners > 2 && (
                    <Box display="flex" alignItems="center" justifyContent="center" style={{ marginLeft: -12, backgroundColor: '#f0f0f0', borderRadius: '50%', width: 24, height: 24 }}>
              ...
              <TableCell>
                <Typography color={row.status.includes("Progress") || completedStatuses.includes(row.status) || row.status === "Incomplete" ? "primary" : "textSecondary"}>
                  {row.status} {(row.status === "Learning in Progress" || (role === "CapDev" && row.status === "Learning in Progress") || completedStatuses.includes(row.status) || row.status === "Incomplete") ? `(${row.completedLearners}/${row.learners} Completed)` : ""}
                </Typography>
              </TableCell>
              <TableCell>
                <IconButton onClick={() => handleArrowClick(row.status)}><ArrowCircleRightOutlinedIcon /></IconButton>
